...
userRouter.post('/reservation',passport.authenticate('jwt',{session : false}),(req,res)=>{
    ...
    reservation.save(err=>{
        if(err)
            ...
            req.user.bookings.push(reservation);
            req.user.save(err=>{
                if(err)
                    res.status(500).json({message : {msgBody : "Error has occured", msgError: true}});
                else
...
userRouter.get('/reservations',passport.authenticate('jwt',{session : false}),(req,res)=>{
    User.findById({_id : req.user._id}).populate('bookings').exec((err,document)=>{
        if(err)
            res.status(500).json({message : {msgBody : "Error has occured", msgError: true}});
        else{
            res.status(200).json({todos : document.bookings, authenticated : true});
        }
    });
});
